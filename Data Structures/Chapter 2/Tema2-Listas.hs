...
{-
   ...
   numéricas son de tipo Int.
      ...
      ( [] ) :: [a] Cualquier tipo, polimórfico

      [ 1 == 1 + 1, -3 < 3 ] :: [Bool]

      [ 1 == 1 + 1, undefined, -3 < 3 ] :: [Bool] undefined tiene todos los tipos, es polimórfico
...
{-
   ...
   y otras son parciales.
      ...
      null :: [a] -> Bool  --comprobar si una lista está vacía(total)

      head :: [a] -> a     --da el primer elemento de la lista(parcial)

      last :: [a] -> a     --da el último elemento de la lista(parcial)

      tail :: [a] -> [a]   --devuelve la lista sin el primer elemento, la cola(parcial)

      init :: [a] -> [a]   --devuelve la lista sin el último elemento(parcial)

      take :: Int -> [a] -> [a]   --devulve la lista solo con los n primeros valores(total)

      drop :: Int -> [a] -> [a]   --devuelve la lista sin los n primeros valores(total)
      ...
      length :: [a] -> Int   --devuelve la longitud de la lista(total)

      elem :: Eq a => a -> [a] -> Bool   --devuelve la respuesta a la pregunta ¿está el dato en la lista?(total)

      notElem :: Eq a => a -> [a] -> Bool  --devuelve lo contrario al anterior

      reverse :: [a] -> [a]   --devuelve la lista al revés(total)

      (++) :: [a] -> [a] -> [a]   --(concatenación) devuelve la unión de las listas(total)
-}
...
prop_disparate x xs =
   [x] ++ xs == xs ++ [x]  --para forzar un tipo en la prueba de quickCheck poner "quickCheck (propiedad :: Tipo -> [Tipo] -> Bool)"", donde la sintaxis viene dada por la forma de definir la función
                           --el test utiliza siempre la tupla vacía, que normalmente funciona en los casos que queramos estudiar
...
{-
   ...
   En Haskell las listas se construyen a partir de dos constructores:
         ...
         2. el operador (:), que sirve para construir una lista a partir de:
                ...
                - su cabeza (primer elemento)  -> la cabeza es de la forma a
                - su cola (lista con el resto de sus elementos)  -> la cola es de la forma [a]
...
-- para saber el tipo de una expresión podemos escribir :t VARIABLE
...
último [] = error "último: no definido"
último [x] = x
...
suma' :: [Int] -> Int
suma' xs = sumaAc xs 0      --0 es solucion del caso base
   where
      sumaAc [] ac = ac
...
longitud' xs = longAc xs 0
   where
      longAc [] ac = ac
      longAc (_:xs) ac = longAc xs (1+ac)
...
inversa' :: [a] -> [a] -- predefinida como reverse
inversa' xs = inversoAc xs []
   where 
      inversoAc [] ac = ac
      inversoAc (x:xs) ac = inversoAc xs (x:ac)
