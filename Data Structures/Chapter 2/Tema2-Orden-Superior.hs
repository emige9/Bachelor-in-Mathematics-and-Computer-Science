...
-- aplicarle ciertas operaciones (+,*,<,...)   estas son específicas del tipo
...
unaFunc :: Int -> Int          --el tipo tendrá la estructura del tipo de la función.
unaFunc = inc
...
{-
   ...
   usando una lambda expresión. (usamos \ como símbolo equivalente a la letra lambda)
-}
...
f :: Int -> Int -> Int -> Int
f x y z = x + 2*y + 3*z              -- f = \ x -> \ y -> \ z -> x + 2*y + 3*z => esto es lo que usa haskell, la sintaxis de tipo y de valor es la misma
g :: Int -> Int
...
apruébame xs = map (max 5) xs           --utilizamos parcialización para la función max
...
-- Ejercicio: define nextChar usando composición de funciones
...
descifrar xs = map (chr . (+(-3)) . ord) xs       -- (\ x -> x - 3)
