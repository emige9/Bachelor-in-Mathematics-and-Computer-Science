...
esPalabra xs = foldr f True xs 
    where 
      f cabeza solCola = ((cabeza >= 'A' && cabeza <= 'Z') || (cabeza >= 'a' ) && (cabeza <= 'z'))
...
pertenece x xs = foldr f False xs                    -- (\ cabeza solCola -> cabeza == x || solCola)
  where
    f cabeza solCola = cabeza==x || solCola

-- |
-- >>> paresImpares [1,2,3,4,5,6,7,8,9,900]
-- ([2,4,6,8],[1,3,5,7,9])

paresImpares :: [Int] -> ([Int], [Int])
paresImpares xs = foldr f ([],[]) xs 
    where 
      f cabeza solCola 
          | even cabeza = (cabeza : (fst solCola),  (snd solCola))
          | otherwise = ((fst solCola), cabeza : (snd solCola))
