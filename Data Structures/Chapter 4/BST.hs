...
delete x (Node y l r)
    ...
    | otherwise = deleteRoot (Node y l r)

deleteRoot :: Ord a => BST a -> BST a
deleteRoot (Node _ l Empty) = l
deleteRoot (Node _ Empty r) = r
deleteRoot (Node _ l r) = Node minimo l (delete minimo r)
    where minimo = minElem r

minElem :: BST a -> a
minElem (Node y Empty _) = y
