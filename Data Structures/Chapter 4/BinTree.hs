...
data TreeB a = EmptyB
             | NodeB a (TreeB a) (TreeB a) deriving Show   --recursividad doble (hijo izquierdo) (hijo derecho)
               --  root  left     right
...
-- saca todos los niveles [atLevelB i tree2 | i <- [0.. (heightB tree2 - 1) ]]
...
pathsToB2 x (NodeB y l r)
    ...
    | otherwise = camino 
      where
        camino = map (y:) ((pathsToB2 x l) ++ (pathsToB2 x r)) 
