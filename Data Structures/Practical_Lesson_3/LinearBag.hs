...
delete x (Node y oy b) 
   | x < y = Node y oy b
   | x==y && oy>1 = Node y (oy-1) b    -- x==y = if oy>1 then Node y (oy-1) b else b
   | x==y = b
...
occurrences_insert_1 x b = occurrences x (insert x b) == (occurrences x b) + 1
...
union :: Ord a => Bag a -> Bag a -> Bag a
union Empty bol1 = bol1
union bol2 Empty = bol2
union bol1 bol2 = undefined 
...
intersection (Node x ox bol1) (Node y oy bol2)
   | x<y = intersection bol1 (Node y oy bol2)
   | x==y = Node x (min ox oy) (intersection bol1 bol2)
   | otherwise = intersection (Node x ox bol1) bol2 
