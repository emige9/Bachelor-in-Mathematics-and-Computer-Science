    ...
    */
    ...
    private static <T extends Comparable<? super T>> Tree<T> merge(Tree<T> h1, Tree<T> h2) {
        ...
        if(h1.elem.compareTo(h2.elem)<=0){
            ArrayList<Tree<T>> arrayHeaps = ganadorYperdedores(h1.left,h1.right, h2);
            heapDeveloper = new Tree<>(h1.elem, arrayHeaps.get(0), merge(arrayHeaps.get(1), arrayHeaps.get(2)));
        } else {
            ArrayList<Tree<T>> arrayHeaps = ganadorYperdedores(h2.left, h2.right, h1);
            heapDeveloper = new Tree<>(h2.elem, arrayHeaps.get(0), merge(arrayHeaps.get(1), arrayHeaps.get(2)));
        }
    ...
    private static <T extends Comparable<? super T>> ArrayList<Tree<T>> ganadorYperdedores(Tree<T> h1, Tree<T> h2, Tree<T> h3) {
