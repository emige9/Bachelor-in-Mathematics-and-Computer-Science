...
merge x Empty = x
merge Empty x = x
merge h1@(Node x n1 l1 r1) h2@(Node y n2 l2 r2)
  | x <= y = Node x (n1+n2) maximo (merge a b)
  | otherwise = Node y (n1+n2) maximo' (merge a' b')
    where 
      (maximo,a,b) = ordena l1 r1 h2
      (maximo',a',b') = ordena l2 r2 h1

ordena :: Heap a -> Heap a -> Heap a -> (Heap a, Heap a, Heap a)
ordena l r h
