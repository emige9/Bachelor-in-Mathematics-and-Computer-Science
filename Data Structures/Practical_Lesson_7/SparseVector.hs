...
get :: Int -> Vector a -> a 
get i (V n t) 
    | (i>=n) || (i<0) = error "Índice incorrecto"
    | (n<=0) = error "Vector vacío" 
    | otherwise = aux i n t 
      where 
        aux i sz (Unif x) = x 
        aux i sz (Node lt rt) 
          | i<medio = aux i medio lt 
          | otherwise = aux (i-medio) medio rt 
            where 
              medio = div sz 2 
...
set i y (V s t)
    | (i>=n) || (i<0) = error "Índice incorrecto"
    | otherwise = (V s (aux i y s t))
      where 
        aux i y sz (Unif x)
          | sz == 1 = (Unif y)
          | (i>=0) && (i<medio) = (Node (aux i y medio (Unif x)) (Unif x))
          | otherwise = (Node (Unif x) (aux (i-medio) y medio (Unif x)))
           where
              medio = div sz 2
        aux i y sz (Node lt rt)
          | i < medio = simplify (aux i y medio lt) rt 
          | otherwise = simplify lt (aux (i-medio) y medio rt)
            where
              medio = div sz 2 
...
mapVector f (V s t) = (V s (aux f t))
  where 
    aux f (Unif x) = (Unif (f x))
    aux f (Node lt rt) = simplify (aux f lt) (aux f rt)
