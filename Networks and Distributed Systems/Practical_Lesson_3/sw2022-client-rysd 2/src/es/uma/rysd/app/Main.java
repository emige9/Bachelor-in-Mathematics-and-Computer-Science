...
public class Main {	
 ...
 // Genera un número entre 0 y max-1 que no haya sido usado previamente (los usados vienen en l)
    ...
    // Pregunta que obtiene dos recursos iguales (vehiculos en este caso) y los compara
    public static void vehiculoMasGrande(SWClient sw){
    	// Obteniendo la cantidad de naves almacenada
    	int max_vehicle = sw.getNumberOfResources("vehicles");
    	if(max_vehicle == 0){
    	...
    	int contador = 0;
    	while(contador < 2){
    		Integer p = getRandomResource(max_vehicle, used);
    		Vehicle vehiculo = sw.getVehicle(sw.generateEndpoint("vehicles", p));
    		if(vehiculo == null){
    	...
    	// Mostrando la información de las naves elegidas
    	for(Vehicle v: vehiculos){
    		System.out.println(v.name + " mide " + v.length);
    	}
    ...
    public static void quienVive1(SWClient sw) throws UnsupportedEncodingException{
